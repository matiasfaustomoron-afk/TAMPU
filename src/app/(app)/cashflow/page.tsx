  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="px-4">
        ...
        >
          ...
          <div className="mt-5 grid grid-cols-3 gap-3 text-white">
            <Mini label={t.dashboard.spent} value={formatCurrency(cashflow.total_spent)} />
            <Mini label={t.dashboard.committed} value={formatCurrency(cashflow.total_committed_future)} />
            <Mini label={t.common.remaining} value={formatCurrency(Math.max(0, cashflow.total_budget - cashflow.total_spent - cashflow.total_committed_future))} />
          </div>
        </IOSFeatureCard>
      </div>
      ...
      <section className="px-4 mt-6">
        <p className="ios-eyebrow">{t.cashflow.status}</p>
        <div className="ios-card p-5 grid grid-cols-3 gap-4">
          <StatChip label={t.cashflow.pace} value={burnStatus === "ok" ? t.cashflow.ok : burnStatus === "warn" ? t.cashflow.careful : t.cashflow.excess} status={burnStatus} />
          <StatChip label={t.cashflow.days} value={dashboard.trip_duration} />
          <StatChip label={t.cashflow.payments} value={money_in_flight.items.length} />
        </div>
      </section>
          ...
          <div className="ios-card p-5">
            ...
            <div className="mt-3 flex gap-4 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-0.5 bg-primary" /> {t.cashflow.actual}
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-px border-t border-dashed border-muted-foreground" /> {t.cashflow.budget}
              </span>
            </div>
          </div>
        </section>
      ...
      {cashflow.by_destination.length > 0 && (
        <IOSSection eyebrow={t.cashflow.byDestination}>
          {cashflow.by_destination.map(d => (
            <div key={d.city} className="ios-list-row">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] text-muted-foreground mt-1">{d.pct}{t.cashflow.pctOfTotal}</p>
              </div>
            </div>
          ))}
        </IOSSection>
      ...
      {money_in_flight.items.length > 0 && (
        <IOSSection eyebrow={t.cashflow.upcomingPayments}>
      ...
      {cashflow.weekly.length > 0 && (
        <section className="px-4 mb-8">
          <p className="ios-eyebrow">{t.cashflow.byWeek}</p>
              ...
              return (
                <div key={w.week_start}>
                  <div className="flex items-center justify-between text-[13px] mb-1">
                    <span className="font-medium">{t.cashflow.weekOf} {w.week_start.slice(5)}</span>
                  ...
                  <div className="flex gap-3 text-[10px] text-muted-foreground mt-1">
                    <span><span className="inline-block w-1.5 h-1.5 rounded-full bg-primary mr-1" />{t.cashflow.expenses} {formatCurrency(w.expenses)}</span>
                    <span><span className="inline-block w-1.5 h-1.5 rounded-full bg-warning mr-1" />{t.cashflow.paymentsLower} {formatCurrency(w.payments_due)}</span>
                  </div>
                </div>
              );
            })}
